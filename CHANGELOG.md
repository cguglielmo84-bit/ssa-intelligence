...
- SAMI mascot images in frontend hero banners (SAMI_News on News Dashboard, SAMI_Research on Home page).
- DOCX export button in all news export dropdowns and article detail modal.
- `export_docx` activity type for news activity tracking.
- Shared DOCX utilities module (`docx-shared.ts`) to reduce duplication between research and news exports.
- Radial gradient overlay on hero banners for better mascot visibility.
- Always show "Selected" section in news export dropdown (disabled at count 0).

### Changed
- Rename news exports to "SAMI News Digest" (title, filenames).
- Rename markdown footer to "Generated by SAMI".
- "Start New Research" button renamed to "New Research".
- "Export" button renamed to "Export News".
