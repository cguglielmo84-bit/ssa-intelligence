...
export function validateCompleteResearch(output) {
    ...
    try {
        ...
        if (research.section1)
            execSummaryOutputSchema.parse(research.section1);
        if (research.section2)
            financialSnapshotOutputSchema.parse(research.section2);
        if (research.section3)
            companyOverviewOutputSchema.parse(research.section3);
        if (research.section4)
            segmentAnalysisOutputSchema.parse(research.section4);
        if (research.section5)
            trendsOutputSchema.parse(research.section5);
        if (research.section6)
            peerBenchmarkingOutputSchema.parse(research.section6);
        if (research.section7)
            skuOpportunitiesOutputSchema.parse(research.section7);
        if (research.section8)
            recentNewsOutputSchema.parse(research.section8);
        if (research.section9)
            conversationStartersOutputSchema.parse(research.section9);
        if (research.section10)
            appendixOutputSchema.parse(research.section10);
        return true;
    }
    catch (error) {
