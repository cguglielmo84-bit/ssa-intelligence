...
export function getOutperformingMetrics(output) {
    const outperforming = [];
    for (const metric of output.peer_comparison_table.metrics) {
        if (typeof metric.company !== 'number')
            continue;
        const peerValues = [
            metric.peer1,
            metric.peer2,
        ...
        if (higherIsBetter.some(m => metric.metric.includes(m))) {
            if (metric.company > avgPeer)
                outperforming.push(metric.metric);
        }
...
//# sourceMappingURL=peer-benchmarking.js.map
