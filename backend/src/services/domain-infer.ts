...
export async function inferDomainForCompany(companyName: string): Promise<string | null> {
  try {
...
export async function ensureDomainForJob(prisma: PrismaClient, jobId: string, companyName: string) {
  try {
    ...
    const inferred = await inferDomainForCompany(companyName);
    const normalized = normalizeDomain(inferred);
